...
//load step value into control register
void    LoadStepValue (unsigned int StepValue)
{
    //declare variables
    unsigned int SPIAddress;
    unsigned int SPIValue;
    ...
    SPIAddress = DRV_CTRL_REG;
    SPIValue = ReadSPI(SPIAddress);
    //clear the present step value
    SPIValue = SPIValue & STEP_MASK;
    //make sure value does not exceed 4 bits
    StepValue = StepValue & 0x000f;
    //shift step value by 3 bits to the left
    StepValue = StepValue << 3;
    //load new torque value
    SPIValue = SPIValue | StepValue;
    WriteSPI(SPIAddress,SPIValue);       
}


//load dead time value into control register
void    LoadDeadTimeValue (unsigned int DeadTimeValue)
{
    //declare variables
    unsigned int SPIAddress;
    unsigned int SPIValue;
    ...
    SPIValue = ReadSPI(SPIAddress);
    //clear the present step value
    SPIValue = SPIValue & DEAD_TIME_MASK;
    //make sure value does not exceed 2 bits
    DeadTimeValue = DeadTimeValue & 0x0003;
    //shift dead time value by 3 bits to the left
    DeadTimeValue = DeadTimeValue << 10;
    //load new dead time value
    SPIValue = SPIValue | DeadTimeValue;
    WriteSPI(SPIAddress,SPIValue);       
}


//load torque value into control register
void    LoadTorqueValue (unsigned int TorqueValue)
{
    //declare variables
    unsigned int SPIAddress;
    unsigned int SPIValue;
    ...
    SPIValue = SPIValue & TORQUE_MASK;
    //make sure value does not exceed 8 bits
    TorqueValue = TorqueValue & 0x00ff;
    //load new torque value
    SPIValue = SPIValue | TorqueValue;
    WriteSPI(SPIAddress,SPIValue);       
}

//load the off time value into the OFF register
void    LoadOffTimeValue (unsigned int OffTimeValue)
{
    //declare variables
    unsigned int SPIAddress;
    unsigned int SPIValue;
    ...
    SPIAddress = DRV_OFF_REG;
    SPIValue = ReadSPI(SPIAddress);
    //clear the present off time value
    SPIValue = SPIValue & OFF_TIME_MASK;
    //make sure value does not exceed 8 bits
    OffTimeValue = OffTimeValue & 0x00ff;
    //load new off time value
    SPIValue = SPIValue | OffTimeValue;
    WriteSPI(SPIAddress,SPIValue);       
}


//load the blank time value into the BLANK register
void    LoadBlankTimeValue (unsigned int BlankTimeValue)
{
    //declare variables
    unsigned int SPIAddress;
    unsigned int SPIValue;
    ...
    SPIAddress = DRV_BLANK_REG;
    SPIValue = ReadSPI(SPIAddress);
    //clear the present blank time value
    SPIValue = SPIValue & BLANK_TIME_MASK;
    //make sure value does not exceed 8 bits
    BlankTimeValue = BlankTimeValue & 0x00ff;
    //load new off time value
    SPIValue = SPIValue | BlankTimeValue;
    WriteSPI(SPIAddress,SPIValue);       
}


//load the adaptive blanking value into the BLANK register
void    LoadAdaptiveBlankingValue (unsigned int AdaptiveBlankingValue)
{
    //declare variables
    unsigned int SPIAddress;
    unsigned int SPIValue;
    ...
    SPIAddress = DRV_BLANK_REG;
    SPIValue = ReadSPI(SPIAddress);
    //clear the present adaptive blank value
    SPIValue = SPIValue & ADAPTIVE_BLANKING_MASK;
    //make sure value does not exceed 1 bits
    AdaptiveBlankingValue = AdaptiveBlankingValue & 0x0001;
    //shift dead time value by 3 bits to the left
    AdaptiveBlankingValue = AdaptiveBlankingValue << 8;
    //load new adaptive blanking value
    SPIValue = SPIValue | AdaptiveBlankingValue;
    WriteSPI(SPIAddress,SPIValue);       
}


//load the decay time value into the DECAY register
void    LoadDecayTimeValue (unsigned int DecayTimeValue)
{
    //declare variables
    unsigned int SPIAddress;
    unsigned int SPIValue;
    
    //get the present value of the decay register
    SPIAddress = DRV_DECAY_REG;
    SPIValue = ReadSPI(SPIAddress);
    //clear the present step value
    SPIValue = SPIValue & DECAY_TIME_MASK;
    //make sure value does not exceed 8 bits
    DecayTimeValue = DecayTimeValue & 0x00ff;
    //load new decay time value
    SPIValue = SPIValue | DecayTimeValue;
    WriteSPI(SPIAddress,SPIValue);       
}


//load the decay mode value into the DECAY register
void    LoadDecayModeValue (unsigned int DecayModeValue)
{
    //declare variables
    unsigned int SPIAddress;
    unsigned int SPIValue;
    ...
    SPIAddress = DRV_DECAY_REG;
    SPIValue = ReadSPI(SPIAddress);
    //clear the present decay mode value
    SPIValue = SPIValue & DECAY_MODE_MASK;
    //make sure value does not exceed 3 bits
    DecayModeValue = DecayModeValue & 0x0007;
    //shift decay mode value by 8 bits to the left
    DecayModeValue = DecayModeValue << 8;
    //load new decay mode value
    SPIValue = SPIValue | DecayModeValue;
    WriteSPI(SPIAddress,SPIValue);       
}


//load the drive time low value into the DRIVE register
void    LoadDriveTimeLowValue (unsigned int DriveTimeLowValue)
{
    //declare variables
    unsigned int SPIAddress;
    unsigned int SPIValue;
    
    //get the present value of the drive register
    SPIAddress = DRV_DRIVE_REG;
    SPIValue = ReadSPI(SPIAddress);
    //clear the present drive time value
    SPIValue = SPIValue & DRIVE_TIME_LOW_MASK;
    //make sure value does not exceed 2 bits
    DriveTimeLowValue = DriveTimeLowValue & 0x0003;
    //shift value by 4 bits to the left
    DriveTimeLowValue = DriveTimeLowValue << 4;
    //load new drive time mode value
    SPIValue = SPIValue | DriveTimeLowValue;
    WriteSPI(SPIAddress,SPIValue);       
}

//load the drive time high value into the DRIVE register
void    LoadDriveTimeHighValue (unsigned int DriveTimeHighValue)
{
    //declare variables
    unsigned int SPIAddress;
    unsigned int SPIValue;
    ...
    SPIValue = ReadSPI(SPIAddress);
    //clear the present drive time value
    SPIValue = SPIValue & DRIVE_TIME_HIGH_MASK;
    //make sure value does not exceed 2 bits
    DriveTimeHighValue = DriveTimeHighValue & 0x0003;
    //shift value by 6 bits to the left
    DriveTimeHighValue = DriveTimeHighValue << 6;
    //load new drive time value
    SPIValue = SPIValue | DriveTimeHighValue;
    WriteSPI(SPIAddress,SPIValue);       
}


//load the drive current low value into the DRIVE register
void    LoadDriveCurrentLowValue (unsigned int DriveCurrentLowValue)
{
    //declare variables
    unsigned int SPIAddress;
    unsigned int SPIValue;
    ...
    SPIAddress = DRV_DRIVE_REG;
    SPIValue = ReadSPI(SPIAddress);
    //clear the present drive current value
    SPIValue = SPIValue & DRIVE_CURRENT_LOW_MASK;
    //make sure value does not exceed 2 bits
    DriveCurrentLowValue = DriveCurrentLowValue & 0x0003;
    //shift value by 8 bits to the left
    DriveCurrentLowValue = DriveCurrentLowValue << 8;
    //load new drive current value
    SPIValue = SPIValue | DriveCurrentLowValue;
    WriteSPI(SPIAddress,SPIValue);       
}


//load the drive current high value into the DRIVE register
void    LoadDriveCurrentHighValue (unsigned int DriveCurrentHighValue)
{
    //declare variables
    unsigned int SPIAddress;
    unsigned int SPIValue;
    ...
    SPIValue = ReadSPI(SPIAddress);
    //clear the present drive current value
    SPIValue = SPIValue & DRIVE_CURRENT_HIGH_MASK;
    //make sure value does not exceed 2 bits
    DriveCurrentHighValue = DriveCurrentHighValue & 0x0003;
    //shift value by 10 bits to the left
    DriveCurrentHighValue = DriveCurrentHighValue << 10;
    //load new drive current value
    SPIValue = SPIValue | DriveCurrentHighValue;
    WriteSPI(SPIAddress,SPIValue);       
}
